...
function toNavItem(entry: DocsEntry, section?: string): DocsNavItem {
  return {
...
export function getEntrySlug(entry: DocsEntry): string {
  return entry.data.slug?.replace(/^\/+|\/+$/g, '') || entry.id;
}
...
function normalizeSidebarItems(
  ...
  return items.flatMap((item) => {
    ...
    if (isAutogenerateItem(item)) {
      return sortNavItems(
        getAutogeneratedEntries(entries, item.autogenerate.directory).map((entry) => toNavItem(entry)),
...
function getSidebarContentItems(sidebar: SidebarConfig, entries: DocsEntry[]): DocsPaginationItem[] {
  const sidebarNav = sidebarConfigToNav(sidebar, entries);
...
export function getEntrySection(entry: DocsEntry, entries: DocsEntry[]): string | undefined {
  for (const sidebar of themeConfig.sidebars) {
    const sectionSlug = getSidebarSectionSlug(sidebar);
    const items = getSidebarContentItems(sidebar, entries);

    if (items.some((item) => item.slug === entry.id)) {
      return sectionSlug;
    }
  }
...
export async function getDocsNavigation(
  ...
  return themeConfig.sidebars.map((sidebar) => {
    const sectionSlug = getSidebarSectionSlug(sidebar);
    const items = getSidebarContentItems(sidebar, entries).map((item, index) => ({
      title: item.label,
      href: item.href,
      slug: item.slug,
      section: sectionSlug,

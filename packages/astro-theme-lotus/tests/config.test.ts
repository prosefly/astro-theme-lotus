...
describe('Lotus config', () => {
  ...
  it('does not load Pagefind for local or disabled search', async () => {
    ...
    await expect(buildPagefindIndex(
      resolveLotusConfig({ search: { provider: 'local' } }),
      new URL('file:///tmp/lotus-pagefind-test/'),
      logger,
      loadPagefindModule,
    )).resolves.toBeUndefined();
    await expect(buildPagefindIndex(
      resolveLotusConfig({ search: false }),
      new URL('file:///tmp/lotus-pagefind-test/'),
      logger,
      loadPagefindModule,
    )).resolves.toBeUndefined();
  ...
  it('loads Pagefind only for the Pagefind provider', async () => {
    ...
    await expect(buildPagefindIndex(
      resolveLotusConfig({ search: { provider: 'pagefind' } }),
      new URL('file:///tmp/lotus-pagefind-test/'),
      { info: () => {} },
      async () => {
        loaded = true;

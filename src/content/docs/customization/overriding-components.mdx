...
| `PageHeader` | Docs article title area. | `title`, `description?`, `sectionTitle?`, `pageActions`, `pageUrl`, `markdownUrl` |
| `PageActions` | Action control inside `PageHeader`. | `actions`, `title`, `pageUrl`, `markdownUrl` |
| `PageNavigation` | Previous and next links below docs content. | `navigation` |
...
## Reuse A Default Shell Component

An override can fully replace the default shell component, or it can reuse the
public default component and add only the markup or classes the project needs.
...
those props when reusing the default shell component so built-in behavior keeps
working.
...
Use `virtual:prosefly/lotus/components/PageActions` inside a shell override
when the override should respect another local `PageActions` override. Use the
public `@prosefly/astro-theme-lotus/components` export when you specifically
want the package default component.

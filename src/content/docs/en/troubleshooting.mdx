...
`configuration/project`, not `docs/en/configuration/project`.

## A generated route is not the URL I expected

Check `docsBase`, `docsLoader({ base })`, locale `directory`, and frontmatter
`slug`. `slug` changes the public URL, but sidebar ownership still follows the
content entry ID.

## The search index returns 404

Confirm `docsBase` and the active locale. With `docsBase: '/docs'`, the root
search index is `/docs/search.json`; localized indexes include the locale
prefix, such as `/docs/zh-cn/search.json`.

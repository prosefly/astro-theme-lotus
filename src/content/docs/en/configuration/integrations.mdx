---
title: Integrations
description: Configure OpenAPI references, Iconify preloading, Expressive Code, Mermaid diagrams, and Markdown transforms.

---

import { Card, CardGrid } from '@prosefly/astro-components';

Lotus includes integrations for icons, syntax highlighting, Mermaid diagrams,
callout directives, CJK-friendly Markdown, image galleries, package-manager
command tabs, and optional OpenAPI reference pages.
...
<CardGrid>
  ...
  <Card icon="lucide:git-branch" title="Mermaid" href="#mermaid">
    Render supported Mermaid code blocks as responsive, themed SVG diagrams at
    build time.
  </Card>
...
Lotus renders supported `mermaid` code blocks as responsive inline SVG during
the Astro build. It is enabled by default, ships no Mermaid runtime to the
browser, and automatically keeps Mermaid source out of syntax highlighting.

See [Mermaid Diagrams](/docs/essentials/mermaid/) for supported diagram types,
authoring syntax, accessibility, and theming.

Disable the transform when Mermaid blocks should remain ordinary code:

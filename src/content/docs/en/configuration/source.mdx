...
Contributors use the same entry file path as edit links. Set `contributors` to
an object when you want to limit or filter the rendered list.
...
`contributors.github` and `contributors.branch` can still override the shared
`source` fields for unusual cases.

...
When the optional OpenAPI integration is configured, add its generated
navigation as an item source:
...
Select a named source when more than one OpenAPI document is configured:
  ...
  items: [
    {
      openapi: {
        ...
        methodBadge: { position: 'leading', variant: 'soft' },
      },
    },
  ],
}
...
Set `introduction` or `methodBadge` to `false` to omit them from navigation.
OpenAPI items can be mixed with manual links and nested inside sidebar groups.
See [OpenAPI](/docs/configuration/integrations/#openapi) for source and route
configuration.

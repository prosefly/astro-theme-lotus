...
import StylePresets from '@/components/StylePresets.astro';
...
Use CSS variable overrides when a project needs a stronger brand direction. The
preset chooser below applies copyable example CSS to the current page while you
preview the result:

<StylePresets />

Preset names are descriptive examples, not Lotus API values. Copy a preset from
`src/styles/presets/` into your project, adjust it, and load it with
`head`:
...
| `--lotus-header-background` | Header background. |
| `--lotus-footer-background` | Footer background. |

...
Use `themeModeControl` to choose the built-in control shown in the header and
mobile menu.
...
Supported values are `segmented-control`, `button`, `select`, `switch`, and
`false`. Set it to `false` to hide the theme mode control while keeping
`appearance.defaultMode` active.

...
Lotus enables Prosefly callout directives by default. Use `note`, `tip`,
`warning`, `caution`, or `danger`; `caution` renders as a warning for Starlight
compatibility.

```md
:::tip[Directive syntax]
Use directives when migrating Markdown from another documentation framework.
:::
```

...
| `DocsContextLayout` | Thin wrapper around `DocsLayout` that resolves docs navigation from `currentSection`. |
...
`DocsLayout` does not calculate navigation. Pass resolved `subnav`, `sidebar`,
and resolved sidebar data to keep the layout reusable across docs, API reference
pages, and other custom collections. Lotus docs routes can use helpers from
`@prosefly/astro-theme-lotus/navigation`; custom collections can create the
same data shape directly.

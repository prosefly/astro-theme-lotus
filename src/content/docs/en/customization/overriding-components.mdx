...
Lotus provides the resolved UI translator on `Astro.locals.t`. Use it in local
overrides when visible shell text should follow the current locale.
...
<a dir={t.dir()} hreflang={locale.lang} href="/docs/search/">
  {t('search.label')}
</a>
```

Use `t.exists('custom.key')` before rendering optional custom strings, or
`t.all()` when an override needs to pass the full resolved dictionary to a
client script.
...
| `PageAside`, `EditThisPage` | Reusing the right sidebar page tools or edit link. |
| `PageMeta`, `Contributors` | Reusing the docs page meta area or its contributors block. |

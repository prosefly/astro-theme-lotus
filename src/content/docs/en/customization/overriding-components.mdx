...
- Use custom theme mode control markup beyond `themeModeControl`.
...
| `ThemeSwitch` | `ThemeModeButton`, `ThemeModeSegmentedControl`, `ThemeModeSelect`, or `ThemeModeSwitch` for custom compositions |
...
Use `themeModeControl` before overriding `ThemeSwitch` if a project only needs
one of the built-in controls.

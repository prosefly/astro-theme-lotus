...
3. **Create the theme config.**
   ...
   Use `src/theme.config.ts` and pass it to `lotus(themeConfig)` when you need
   TypeScript, imports, or dynamic values. Options passed to `lotus({...})`
   override values from `theme.config.json`.

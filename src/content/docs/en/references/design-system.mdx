...
Typography inside Markdown is styled through Tailwind Typography's `.prose`
class. Lotus maps its semantic tokens into that layer and adds focused styles
for heading anchors, task lists, footnotes, embedded media, and code frames.
Use `.not-prose` for component surfaces that should opt out of article styles.

...
| `--lotus-header-background` | Header background. Defaults to `--lotus-background`. |
| `--lotus-footer-background` | Footer background. Defaults to `--lotus-background`. |
...
| `--lotus-code-background` | Inline code and code body background. |
| `--lotus-code-surface` | Code frame header, tab bar, and toolbar surface. |
...
`.prose` maps Tailwind Typography to a small set of Lotus extension tokens.
...
Expressive Code frame chrome, such as headers, tab bars, and terminal toolbars,
uses `--lotus-code-surface` through Lotus prose CSS.

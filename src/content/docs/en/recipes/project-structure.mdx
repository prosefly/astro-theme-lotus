...
- astro.config.ts registers the Lotus integration
- src
  - content.config.ts registers content from `./src/content`
  - theme.config.ts enables `homepage: true`
  - content
...
This structure needs `docsLoader({ base: './src/content' })` so the top-level
`index.mdx` belongs to the docs collection.
...
- src
  ...
  - content
    - index.mdx optional site homepage
    - docs
      - en
        - index.mdx
        - overview.mdx
...
2. **Add locale directories only when translations exist.**
   ...
   move pages under directories such as `docs/en` and `docs/zh-cn`.
